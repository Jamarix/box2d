...
#include <box2d/b2VoronoiDiagram.h>
//#include <Box2D/Particle/b2ParticleAssembly.h>
#include <box2d/b2ParticleAssembly.h>
...
b2ParticleSystem::b2ParticleSystem(const b2ParticleSystemDef* def,
	...
	m_stuckParticleBuffer(world->GetBlockAllocator()),
	m_proxyBuffer(world->GetBlockAllocator()),
	m_contactBuffer(world->GetBlockAllocator()),
	m_bodyContactBuffer(world->GetBlockAllocator()),
	m_pairBuffer(world->GetBlockAllocator()),
	m_triadBuffer(world->GetBlockAllocator())
{
	b2Assert(def);
	m_paused = false;
	m_timestamp = 0;
...
template <typename T> void b2ParticleSystem::FreeBuffer(T** b, int capacity)
{
	...
	m_world->GetBlockAllocator().Free(*b, sizeof(**b) * capacity);
	*b = NULL;
}
	...
	T* oldBuffer, int32 oldCapacity, int32 newCapacity)
{
	...
	T* newBuffer = (T*) m_world->GetBlockAllocator().Allocate(
		sizeof(T) * newCapacity);
	if (oldBuffer)
	{
		memcpy(newBuffer, oldBuffer, sizeof(T) * oldCapacity);
		m_world->GetBlockAllocator().Free(oldBuffer, sizeof(T) * oldCapacity);
	}
	return newBuffer;
}
...
template <typename T> T* b2ParticleSystem::RequestBuffer(T* buffer)
{
	if (!buffer)
	{
		...
		buffer = (T*) (m_world->GetBlockAllocator().Allocate(
	...
	const b2ParticleGroupDef& groupDef)
{
	...
	void* mem = m_world->GetBlockAllocator().Allocate(sizeof(b2ParticleGroup));
...
void b2ParticleSystem::SplitParticleGroup(b2ParticleGroup* group)
{
	...
	ParticleListNode* nodeBuffer =
		(ParticleListNode*) m_world->GetStackAllocator().Allocate(
	...
	m_world->GetStackAllocator().Free(nodeBuffer);
}
	...
	int32 firstIndex, int32 lastIndex, const ConnectionFilter& filter)
{
	...
	if (particleFlags & k_triadFlags)
	{
		b2VoronoiDiagram diagram(
			&m_world->GetStackAllocator(), lastIndex - firstIndex);
		for (int32 i = firstIndex; i < lastIndex; i++)
		{
...
void b2ParticleSystem::DestroyParticleGroup(b2ParticleGroup* group)
{
	...
	m_world->GetBlockAllocator().Free(group, sizeof(b2ParticleGroup));
}
...
void b2ParticleSystem::ComputeDepth()
{
	b2ParticleContact* contactGroups = (b2ParticleContact*) m_world->
		GetStackAllocator().Allocate(sizeof(b2ParticleContact) * m_contactBuffer.GetCount());
	...
	b2ParticleGroup** groupsToUpdate = (b2ParticleGroup**) m_world->
		GetStackAllocator().Allocate(sizeof(b2ParticleGroup*) * m_groupCount);
	...
	m_world->GetStackAllocator().Free(groupsToUpdate);
	m_world->GetStackAllocator().Free(contactGroups);
}
...
inline b2ContactFilter* b2ParticleSystem::GetParticleContactFilter() const
{
	return (m_allParticleFlags & b2_particleContactFilterParticle) ?
		m_world->GetContactManager().m_contactFilter : NULL;
}
...
inline b2ContactListener* b2ParticleSystem::GetParticleContactListener() const
{
	return (m_allParticleFlags & b2_particleContactListenerParticle) ?
		m_world->GetContactManager().m_contactListener : NULL;
}
...
void b2ParticleSystem::UpdateContacts(bool exceptZombie)
{
	...
	b2ParticlePairSet particlePairs(&m_world->GetStackAllocator());
...
void b2ParticleSystem::UpdateBodyContacts()
{
	...
	FixtureParticleSet fixtureSet(&m_world->GetStackAllocator());
...
void b2ParticleSystem::SolveZombie()
{
	...
	int32 newCount = 0;
	int32* newIndices = (int32*) m_world->GetStackAllocator().Allocate(
	...
	m_count = newCount;
	m_world->GetStackAllocator().Free(newIndices);
	...
	UserOverridableBuffer<T>* buffer, T* newData, int32 newCapacity)
{
	...
	{
		m_world->GetBlockAllocator().Free(
								  ...
								  const b2Vec2& force)
{
	...
	float d = (float)(lastIndex - firstIndex);
	const b2Vec2 distributedForce = b2Vec2(force.x / d, force.y / d);
										  ...
										  const b2Vec2& impulse)
{
	...
	const b2Vec2 velocityDelta = b2Vec2(impulse.x / totalMass, impulse.y / totalMass);
	for (int32 i = firstIndex; i < lastIndex; i++)
	{
